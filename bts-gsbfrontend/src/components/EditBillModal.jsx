...
export default function EditBillModal({ bill, isOpen, onClose, onSave }) {
    ...
    try {
      const billData = {
        metadata: {
          ...
          status: bill.status // Keep the existing status
        },
        proof: data.proof
      };

      const updatedBill = await authAPI.updateBill(bill._id, billData);
      onSave(updatedBill);
      onClose();
    } catch (error) {
              ...
              <div className="sm:flex sm:items-start">
                <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-medium leading-6 text-gray-900">Edit Bill</h3>
                    <span className="text-sm text-gray-500">ID: #{bill._id}</span>
                  </div>
                  ...
                  <div className="mt-4 space-y-6">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">
                        Receipt (optional - leave empty to keep current)
                      </label>
                      <div
                      ...
                      >
                        <div className="space-y-1 text-center">
                          ...
                          {data.proof && (
                            <div className="mt-2">
                              <p className="text-sm text-green-600 mb-2">New file selected:</p>
                              <img src={URL.createObjectURL(data.proof)} alt="New Receipt" className="max-w-full h-auto" />
                            </div>
                          )}
                        </div>
                      </div>
